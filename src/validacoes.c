...
int valida_crn(char *crn) {
    ...
    if (crn[0] != 'C' || crn[1] != 'R' || crn[2] != 'N' || crn[3] != '-' || !isdigit(crn[4])
        || crn[5] != '/' || !isdigit(crn[6]) || !isdigit(crn[7]) || !isdigit(crn[8])
...
int validar_nome(const char *nome) {
    ...
    for (int i = 0; i < tamanho; i++) {
        ...
        if (!(isalpha(c) || c == ' '
              || strchr("áàâãäéèêëíìîïóòôõöúùûüçÁÀÂÃÄÉÈÊËÍÌÎÏÓÒÔÕÖÚÙÛÜÇ", c))) {
            return 0;
        }
    }
...
int validar_observacao(const char *obs) {
    ...
    for (int i = 0; i < tamanho; i++) {
        ...
        if (isalpha(c) || isdigit(c) || c == ' ' || c == '%'
            || strchr(
                ".,!?;:-_()[]{}\"'@#$/\\+=*&°ºª|~^´`áàâãäéèêëíìîïóòôõöúùûüçÁÀÂÃÄÉÈÊËÍÌÎÏÓÒÔÕÖÚÙÛÜÇ",
                c)) {
            continue;
        }
